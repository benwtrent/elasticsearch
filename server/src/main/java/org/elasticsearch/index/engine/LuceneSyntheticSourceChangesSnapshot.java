    ...
    private Translog.Operation createOperation(
        ...
        if (docRecord.isTombstone() && fieldLoader.id() == null) {
            ...
            return new Translog.Index(
                ...
                fieldLoader.routing(),
                readSlice(context.reader(), segmentDocID),
                -1 // autogenerated timestamp
            );
        }
    }

    private static String readSlice(org.apache.lucene.index.LeafReader reader, int docId) throws IOException {
        FieldInfo fi = reader.getFieldInfos().fieldInfo(SliceFieldMapper.NAME);
        if (fi == null || fi.getDocValuesType() != DocValuesType.SORTED) {
            return null;
        }
        var dv = reader.getSortedDocValues(SliceFieldMapper.NAME);

...
public interface VectorSimilarityFunctions {
     ...
     * vector data bytes. The third argument is the length of the vector data.
     */
    MethodHandle dotProductHandle8s();

    /**
     * Produces a method handle which computes the dot product of several byte (signed
     * int8) vectors. This bulk operation can be used to compute the dot product between a
     * single query vector and a subset of vectors from a dataset (array of vectors). Each
     * vector to include in the operation is identified by an offset inside the dataset.
     *
     * <p> signed int8 byte vectors have values in the range of -128 to 127 (inclusive).
     ...
     * </ol>
     */
    MethodHandle dotProductHandle8sBulkWithOffsets();

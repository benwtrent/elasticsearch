    ...
    void waitForTaskRemoved(String modelId,
        ...
        trainedModelAllocationService.waitForAllocationCondition(
            ...
            allocation -> {
                ...
                // Indicates that the nodes have canceled their local tasks
                // Since there is no flag for allocations being deleted, new nodes being added to the cluster
                // could spin up the node undesirably
                // TODO add flag for overall allocation state
                return Sets.intersection(nodesOfConcern, allocation.getNodeRoutingTable().keySet()).isEmpty();
            },
            request.getTimeout(),
            new TrainedModelAllocationService.WaitForAllocationListener() {
                @Override
                public void onResponse(TrainedModelAllocation trainedModelAllocation) {

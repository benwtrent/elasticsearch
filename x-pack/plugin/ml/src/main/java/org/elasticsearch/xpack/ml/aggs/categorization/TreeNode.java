...
abstract class TreeNode implements ToXContentObject, Accountable {
    ...
    static class LeafTreeNode extends TreeNode {
        ...
        public long ramBytesUsed() {
            return Double.BYTES + sizeOfCollection(logGroups);
        }
        ...
        public XContentBuilder toXContent(XContentBuilder builder, Params params) throws IOException {
            ...
            builder.field("log_groups", logGroups);
            builder.endObject();
            return builder;
        }
        ...
        Optional<LogGroup> getAndUpdateLogGroup(BytesRef[] logTokens, long docCount) {
            return getBestLogGroup(logTokens)
                .map(bestGroupAndSimilarity -> {
    ...
    static class InnerTreeNode extends TreeNode {
        ...
        private static final long SHALLOW_SIZE = RamUsageEstimator.shallowSizeOfInstance(InnerTreeNode.class);
        ...
        @Override
        public LogGroup getLogGroup(final BytesRef[] logTokens) {
            return getChild(logTokens[childrenTokenPos])
                .or(() -> getChild(WILD_CARD))
        ...
        @Override
        public long ramBytesUsed() {
            return SHALLOW_SIZE + sizeOfMap(children, 0) + (long)smallestChild.size() * NUM_BYTES_OBJECT_REF;
        }
        ...
        public LogGroup addLog(final BytesRef[] logTokens, final long docCount, final TreeNodeFactory treeNodeFactory) {
            ...
            TreeNode child = getChild(currentToken)
                .map(node -> {
                ...
                .orElseGet(() -> {
                    if (docCount > 1) {
                        LOGGER.trace(() -> new ParameterizedMessage("got a token [{}] with doc_count [{}] percentage [{}]",
                            logTokens[childrenTokenPos].utf8ToString(), docCount, (double)docCount/docCount));
                    }
        ...
        TreeNode addChild(BytesRef token, TreeNode node) {
            if (node == null || token == null) {
                return null;
            }
            Optional<TreeNode> existingChild = getChild(token).map(
                existingNode -> {
        ...
        Optional<TreeNode> getChild(BytesRef token) {
        ...
        public XContentBuilder toXContent(XContentBuilder builder, Params params) throws IOException {
            ...
            for (Map.Entry<BytesRef, TreeNode> entry : children.entrySet()) {

 ...
 * These tests verify that {@code METADATA _slice} can be requested, kept,
 * and filtered (with pushdown to Lucene) against indices with
 * {@code index.slice.enabled: true}.
 */
...
public class MetadataSliceIT extends ESRestTestCase {
     ...
     * Without the flag, {@code index.slice.enabled} is rejected and the
     * {@code _slice} metadata attribute is not registered.
     */
    @ClassRule
    ...
    /**
     * Creates a slice-enabled index, indexes a few documents spread across two
     * slice values, then verifies that {@code FROM idx METADATA _slice | KEEP _slice}
     * returns the correct slice values for every document.
     */
    ...
    /**
     * Verifies that {@code WHERE _slice == "s1"} is pushed down to Lucene and
     * only returns documents whose routing matches the given slice value.
     */
    ...
    /**
     * Verifies that {@code WHERE _slice != "s1"} is pushed down to Lucene and
     * returns only documents with a different slice value.
     */
    ...
    /**
     * Verifies that {@code WHERE _slice LIKE "s1*"} is pushed to Lucene via a doc-values
    ...
    /**
     * Verifies that {@code WHERE _slice RLIKE "s[12]"} is pushed to Lucene via a doc-values
    ...
    /**
     * Verifies that {@code WHERE _slice == "s1" OR _slice == "s2"} is optimized to an
     * {@code In(_slice, ["s1","s2"])} expression and the shard routing includes both values.
     */
    public void testWhereSliceOrFilterRoutes() throws IOException {
    ...
    /**
     * Verifies that a conflicting AND ({@code _slice == "s1" AND _slice == "s2"}) is
     * optimized to zero results — no document can satisfy both routing constraints.
     */
     ...
     * {@code index.slice.validated} must be set at index creation time;
     * they are {@code Final} settings and cannot be changed afterwards.
     */
    ...
    /**
     * Indexes a single document using the given routing value as its {@code _slice}.
     * In slice mode the routing parameter IS the slice value; it is stored as
     * sorted doc values under the {@code _routing} field.
     */
